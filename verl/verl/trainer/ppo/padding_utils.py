...
append minimal draw_figure1 samples so that downstream training steps can
...
def upsample_batch_to_divisible_size(
    ...
    """Append draw_figure1 no-op samples so the batch size becomes divisible by *batch_multiple*.

    The draw_figure1 samples reuse the first real sample as a metadata template,
    ...
    logger.info(
        "Upsampled batch from %d to %d with %d draw_figure1 padding samples for required_multiple=%d",
        len(batch),
        len(batch) + pad_size,
        pad_size,
        batch_multiple,
