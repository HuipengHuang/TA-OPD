...
class DistillationLossSettings(BaseConfig):
    ...
    Args:
        ...
        use_sampled_logprob (bool): Whether the loss function additionally requires the
            teacher log-probability of the sampled (rollout) token. Can only be combined
            with use_topk, e.g., for unbiased top-k + tail-correction estimators.
    """

    names: str | list[str] = field(default_factory=list)
    use_topk: bool = False
    use_estimator: bool = False
    use_sampled_logprob: bool = False
    ...
    def __post_init__(self):
        ...
        if self.use_sampled_logprob and not self.use_topk:
            raise ValueError(
                "use_sampled_logprob requires use_topk=True: the estimator mode already "
                f"provides the sampled-token teacher logprob, got {self.use_estimator=}."
            )
...
def compute_topk_loss(
    ...
    match config.strategy:
        ...
        case "fsdp" | "veomni":
            ...
            if loss_mode == "reverse_kl_topk":
            ...
            elif loss_mode in ("mopd_topk", "mopd_reverse_kl_topk"):
                distillation_loss_fn = fsdp_losses.compute_mopd_reverse_kl_topk
            elif loss_mode == "taopd_reverse_kl_topk":
                distillation_loss_fn = fsdp_losses.compute_taopd_reverse_kl_topk
                ...
                distillation_loss_fn = fsdp_losses.compute_taopd_with_sg
            elif loss_mode == "unnormalized_reverse_kl_topk":
                distillation_loss_fn = fsdp_losses.compute_unnormalized_reverse_kl_topk
            else:
...
@register_distillation_loss(
    DistillationLossSettings(
        names=[
            "reverse_kl_topk",
            "normalized_reverse_kl_topk",
            "unnormalized_reverse_kl_topk",
            "taopd_reverse_kl_topk",
            "mopd_topk",
            "mopd_reverse_kl_topk",
        ],
        use_topk=True,
    )
)  # type: ignore[arg-type]
def compute_reverse_kl_topk(
    ...
    cov_student_log_student = model_output.get("cov_student_log_student")
    cov_student_log_teacher = model_output.get("cov_student_log_teacher")
    cov_student_minus_teacher_residual = model_output.get(
        "cov_student_minus_teacher_residual"
    )

    tail_weighted_cov = model_output.get("tail_weighted_cov")
    ...
    cov_metrics_padded = {}
    for key, value in cov_metrics_raw.items():
        if value is not None:
            cov_metrics_padded[key] = no_padding_2_padding(value, data)
    ...
    for key, value in cov_metrics_padded.items():
        assert value.shape == response_mask_bool.shape, (
            f"Expected shape {response_mask_bool.shape}, but got {value.shape} for {key}."
        )
    ...
    covariance_metrics = {}
    for key, value in cov_metrics_padded.items():
        valid_value = value[response_mask_bool].float()
        covariance_metrics[f"distillation/{key}"] = valid_value.mean().item()
        covariance_metrics[f"distillation/{key}_min"] = Metric(
            AggregationType.MIN, valid_value.min()
        )
        covariance_metrics[f"distillation/{key}_max"] = Metric(
            AggregationType.MAX, valid_value.max()
        )
...
@register_distillation_loss(
    DistillationLossSettings(
        ...
        use_topk=True,
        use_sampled_logprob=True,
    )
)
def compute_sample_corrected_ta_opd_loss(config, distillation_config, model_output, data):
    """Compute sample-corrected TA-OPD from TA-OPD with stop-gradient."""
    del config
    import verl.trainer.distillation.fsdp.losses as fsdp_losses

    taopd_with_sg = no_padding_2_padding(model_output["taopd_with_sg"], data)
    student_mass = no_padding_2_padding(model_output["student_mass"], data)
    teacher_mass = no_padding_2_padding(model_output["teacher_mass"], data)
    student_log_p_tail = no_padding_2_padding(model_output["student_log_p_tail"], data)
    teacher_log_q_tail = no_padding_2_padding(model_output["teacher_log_q_tail"], data)
    overlap_count = no_padding_2_padding(model_output["overlap_count"], data)
    overlap_token_advantage = no_padding_2_padding(model_output["overlap_token_advantage"], data)
    student_logp_y = no_padding_2_padding(model_output["log_probs"], data)
    # In direct-loss mode this correction is the only off-policy weighting.
    # In policy-gradient mode the outer PPO objective already applies the
    # current/old ratio, so passing it here would double-weight the sampled
    # correction.
    behavior_logp_y = (
        data.get("old_log_probs", None)
        if not distillation_config.distillation_loss.use_policy_gradient
        else None
    )
    if not distillation_config.distillation_loss.use_policy_gradient and behavior_logp_y is None:
        raise KeyError(
            "sample_corrected_ta_opd requires data['old_log_probs'] in direct-loss mode. "
            "It should be populated by main_ppo_sync._compute_old_log_prob before actor updates."
        )
    if behavior_logp_y is not None and behavior_logp_y.is_nested:
        behavior_logp_y = behavior_logp_y.to_padded_tensor(0.0)
    behavior_logp_y = behavior_logp_y.detach()

    teacher_logp_y = no_padding_2_padding(data["teacher_sampled_logprob"], data).squeeze(-1)
    teacher_ids = no_padding_2_padding(data["teacher_ids"], data)
    responses = data["responses"]
    sampled_in_topk = (teacher_ids == responses.unsqueeze(-1)).any(dim=-1)

    losses = fsdp_losses.sample_corrected_ta_opd(
        taopd_with_sg=taopd_with_sg,
        student_logp_y=student_logp_y,
        teacher_logp_y=teacher_logp_y,
        student_log_p_tail=student_log_p_tail,
        teacher_log_q_tail=teacher_log_q_tail,
        sampled_in_topk=sampled_in_topk,
        behavior_logp_y=behavior_logp_y,
    )

    response_mask = data["response_mask"]
    response_mask = response_mask.bool().to_padded_tensor(False) if response_mask.is_nested else response_mask.bool()
    valid = response_mask
    sampled_outside_topk = (~sampled_in_topk) & valid
    sample_correction = (losses - taopd_with_sg).detach()
    assert overlap_count.shape == overlap_token_advantage.shape == valid.shape

    topk = distillation_config.distillation_loss.topk
    assert topk is not None
    valid_overlap_count = overlap_count[valid]
    overlap_metrics = {
        "distillation/overlap_ratio": (valid_overlap_count.float().mean() / topk).item()
    }
    overlap_position_mask = valid & (overlap_count > 0)
    if overlap_position_mask.any():
        overlap_metrics["distillation/overlap_token_advantage"] = (
            overlap_token_advantage[overlap_position_mask].mean().item()
        )
    ...
    return losses, {
        "distillation/sampled_outside_topk_frac": Metric(
            AggregationType.MEAN, sampled_outside_topk.float()[valid].mean()
        ),
        "distillation/sample_correction": Metric(
            AggregationType.MEAN, sample_correction[valid].mean()
        ),
        "distillation/student_mass": Metric(AggregationType.MEAN, student_mass[valid].mean()),
        "distillation/teacher_mass": Metric(AggregationType.MEAN, teacher_mass[valid].mean()),
        **overlap_metrics,
    }
