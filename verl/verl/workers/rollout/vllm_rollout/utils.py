...
def extract_prompt_logprobs(output: RequestOutput, num_prompt_logprobs: Optional[int], result_dict: dict[str, list]):
    ...
    prompt_logprobs_ls, prompt_ids_ls = [], []
    prompt_sampled_logprobs_ls = []
    # NOTE: logprob of first prompt token is None.
    # prompt_logprobs[i] is the distribution at position i predicting prompt_token_ids[i],
    # so after slicing [1:], position j corresponds to actual token prompt_token_ids[j + 1].
    actual_token_ids = output.prompt_token_ids[1:]
    assert len(actual_token_ids) == len(output.prompt_logprobs) - 1

    for actual_token_id, logprobs_dict in zip(actual_token_ids, output.prompt_logprobs[1:]):
        if num_prompt_logprobs == 0:
            token_id_str = list(logprobs_dict.keys())[0]
            logprob = logprobs_dict[token_id_str].logprob
            prompt_logprobs_ls.append([logprob])
            prompt_ids_ls.append([int(token_id_str)])
            # With prompt_logprobs=0, the single entry IS the actual token.
            prompt_sampled_logprobs_ls.append(logprob)
        else:
            prompt_ids = [None] * num_prompt_logprobs
            prompt_logprobs = [None] * num_prompt_logprobs
            # We get either top-k logprobs or top-k plus the sampled logprob (if sampled token is not in top-k)
            ...
            prompt_logprobs_ls.append(prompt_logprobs)
            prompt_ids_ls.append(prompt_ids)
            # vLLM guarantees the actual prompt token is always present in logprobs_dict
            # (either within top-k, or as the extra (k+1)-th entry).
            assert actual_token_id in logprobs_dict, (
                f"actual token {actual_token_id} missing from prompt logprobs dict"
            )
            prompt_sampled_logprobs_ls.append(logprobs_dict[actual_token_id].logprob)

    # NOTE: pad a dummy prompt logprob for last prompt token.
    prompt_logprobs_ls.append([0.0] * max(num_prompt_logprobs, 1))
    prompt_ids_ls.append([0] * max(num_prompt_logprobs, 1))
    prompt_sampled_logprobs_ls.append(0.0)

    result_dict["prompt_ids"] = prompt_ids_ls
    result_dict["prompt_logprobs"] = prompt_logprobs_ls
    result_dict["prompt_sampled_logprob"] = prompt_sampled_logprobs_ls