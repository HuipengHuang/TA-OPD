...
def left_right_2_no_padding(data: TensorDict) -> TensorDict:
    ...
    teacher_sampled_logprob = data.get("teacher_sampled_logprob", None)
    ...
    #     print(f"[probe D] {tuple(teacher_sampled_logprob.shape)}")  # 期望 (b, s, 1)

    if teacher_sampled_logprob is not None:
        teacher_sampled_logprob_rmpad = index_first_axis(
            teacher_sampled_logprob.unsqueeze(-1).flatten(0, 1), indices
        )
        teacher_sampled_logprob_nested = torch.nested.nested_tensor_from_jagged(
            teacher_sampled_logprob_rmpad.squeeze(-1), offsets=cu_seqlens
        )
        data["teacher_sampled_logprob"] = teacher_sampled_logprob_nested
        # print(f"padding: {teacher_sampled_logprob.shape}")
